...
DEVICE_GPU	=	(DEVICEREGS_BEGIN + 0x10)	; 16 bytes
DEVICE_SND	=	(DEVICEREGS_BEGIN + 0x20)	; 16 bytes

GPU_WRITEBYTE	=	(DEVICE_GPU + 0x0)	; 1 byte
GPU_READBYTE	=	(DEVICE_GPU + 0x1)	; 1 byte
GPU_WRITEADDR	=	(DEVICE_GPU + 0x2)	; 2 bytes
GPU_OFFSET	=	(DEVICE_GPU + 0x4)	; 2 bytes
GPU_VMODE	=	(DEVICE_GPU + 0x6)	; 1 byte

;	SND_WAVE	=	(DEVICE_SND + 0x0)	; 1 byte
;	SND_VOL		=	(DEVICE_SND + 0x1)	; 2 bytes
;	SND_DUTY	=	(DEVICE_SND + 0x3)	; 2 bytes
;	SND_CH1_NOTE	=	(DEVICE_SND + 0x5)	; 1 byte
;	SND_CH1_BEND	=	(DEVICE_SND + 0x6)	; 1 byte
;	SND_CH2_NOTE	=	(DEVICE_SND + 0x7)	; 1 byte
;	SND_CH2_BEND	=	(DEVICE_SND + 0x8)	; 1 byte
;	SND_CH3_NOTE	=	(DEVICE_SND + 0x9)	; 1 byte
;	SND_CH3_BEND	=	(DEVICE_SND + 0xA)	; 1 byte
;	SND_CH4_NOTE	=	(DEVICE_SND + 0xB)	; 1 byte
;	SND_CH4_BEND	=	(DEVICE_SND + 0xC)	; 1 byte
;	SND_WRITEADDR	=	(DEVICE_SND + 0xD)	; 1 byte
;	SND_WRITEBYTE	=	(DEVICE_SND + 0xE)	; 1 byte
;	SND_WRITE	=	SND_WRITEBYTE
SND_CH0_PITCH	=	(DEVICE_SND + 0x0)	; 2 bytes
SND_CH0_VOL	=	(DEVICE_SND + 0x2)	; 1 byte
SND_CH0_WAVE	=	(DEVICE_SND + 0x3)	; 1 byte

SND_CH1_PITCH	=	(DEVICE_SND + 0x4)	; 2 bytes
SND_CH1_VOL	=	(DEVICE_SND + 0x6)	; 1 byte
SND_CH1_WAVE	=	(DEVICE_SND + 0x7)	; 1 byte

SND_CH2_PITCH	=	(DEVICE_SND + 0x8)	; 2 bytes
SND_CH2_VOL	=	(DEVICE_SND + 0xA)	; 1 byte
SND_CH2_WAVE	=	(DEVICE_SND + 0xB)	; 1 byte

SND_CH3_PITCH	=	(DEVICE_SND + 0xC)	; 2 bytes
SND_CH3_VOL	=	(DEVICE_SND + 0xE)	; 1 byte
SND_CH3_WAVE	=	(DEVICE_SND + 0xF)	; 1 byte
